...
async function refreshState() {
  ...
  records = Object.values(state.records).sort((left, right) => timestamp(right.updatedAt) - timestamp(left.updatedAt));
...
async function analyze() {
  ...
  try {
    ...
    await repository.setCurrentAnalysis(current);
    setStatus("分析完成。");
  } catch (error) {
    current = null;
...
async function softDelete(id) {
  ...
  if (!record) return;
  await repository.softDelete(id, nowIso());
  await refreshState(); renderLibrary(); renderTrash();
}

async function restore(id) {
  const record = records.find(item => item.id === id);
  if (!record) return;
  await repository.restore(id, nowIso());
  await refreshState(); renderLibrary(); renderTrash();
}

async function forever(id) {
  if (!confirm("確定永久刪除？永久刪除後無法復原。")) return;
  await repository.permanentlyDelete(id, nowIso());
  await refreshState(); renderTrash(); renderLibrary();
}
...
function exportJson() {
  ...
  setSettingsMessage(`已匯出 ${records.length} 筆收藏與垃圾桶資料。`);
}
...
async function importJson(file) {
  try {
    ...
    await refreshState(); renderLibrary(); renderTrash();
    setSettingsMessage(`匯入完成，共保留 ${records.length} 筆唯一資料；較新的本機資料未被覆蓋。`);
  } catch (error) {
    setSettingsMessage(`匯入失敗：${error.message}`, true);
  } finally {
    $("importFile").value = "";
  }
}
...
async function loadSpeechSettings() {
  ...
  $("voiceSelect").innerHTML = availableVoices.length
    ? availableVoices.map(voice => `<option value="${esc(voice.voiceURI)}">${esc(voice.name)}</option>`).join("")
    : '<option value="">暫無 en-US 聲音</option>';
  if (selected) $("voiceSelect").value = selected.voiceURI;
}

async function saveSpeechSettings() {
  await integration.setDevicePreferences({ speechRate: $("speedSelect").value, voiceURI: $("voiceSelect").value });
}

async function runSync() {
  $("syncStatus").textContent = "正在同步";
  try { await syncService.sync(); $("syncStatus").textContent = `已同步：${formatTime(nowIso())}`; }
...
function bindUi() {
  ...
  $("clearBtn").addEventListener("click", async () => {
    current = null;
    await repository.setCurrentAnalysis(null);
    $("inputText").value = "";
  ...
  $("speedSelect").addEventListener("change", saveSpeechSettings);
  $("voiceSelect").addEventListener("change", saveSpeechSettings);
  $("previewVoiceBtn").addEventListener("click", () => play("Hello, welcome to My English."));
  $("enableSyncBtn").addEventListener("click", runSync);
  $("syncNowBtn").addEventListener("click", runSync);
  $("pauseSyncBtn").addEventListener("click", () => { $("syncStatus").textContent = "同步已暫停"; });
  $("disableSyncBtn").addEventListener("click", () => { syncService.provider.disconnect(); $("syncStatus").textContent = "同步已關閉"; });
  window.addEventListener("online", () => { if (syncService?.provider.connected) runSync(); });
}
...
async function bootstrap() {
  try {
    ...
    syncService = new MyEnglishSync({ repository, provider, deviceId: initialized.deviceId });
    await refreshState();
    await loadSpeechSettings();
    globalThis.speechSynthesis?.addEventListener?.("voiceschanged", loadSpeechSettings);
    $("syncStatus").textContent = provider.configured ? "同步尚未開啟" : "同步尚未設定 Bonnie OS Client ID";
    bindUi();
    renderLibrary();
    renderTrash();
  } catch (error) {
    console.error("Bonnie OS Data Layer 初始化失敗。", error);
    setStatus(`資料初始化失敗：${error?.message || "未知錯誤"}。v1.2 原始資料仍保留，請勿清除瀏覽器資料。`, true);
  }
}
